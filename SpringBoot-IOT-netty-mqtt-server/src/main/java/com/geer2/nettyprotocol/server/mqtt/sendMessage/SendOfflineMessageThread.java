...
import com.geer2.nettyprotocol.server.bean.forbusiness.UpMessage;
import com.geer2.nettyprotocol.server.mqtt.channel.AbstractMqttHandlerServerImpl;
import com.geer2.nettyprotocol.server.mqtt.handler.HttpServerHandler;
import com.geer2.nettyprotocol.util.Constants;
    ...
    public void run()
    {
        ...
        if (DeviceManage.DEVICE_MAP.containsKey(stb_code))
        {
            ...
            if (ctxx != null && ctxx.channel().isActive())
            {
                ...
                pubMsg = AbstractMqttHandlerServerImpl.buildPublish(GsonJsonUtil.toJson(msg.getMsgInfo()), Constants.TOPIC_STB, 1);
